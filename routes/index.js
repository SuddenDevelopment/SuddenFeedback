...
var arrWordSets=[
    {
        name:"Obama Themes"
        ,phrases:["Illuminati","Hitler","worst president","Thanks Obama"]
    }
];
/*
 * GET home page.
            ...
            twit.stream('statuses/filter', {track: ['obama']}, function (stream) {
                stream.on('data', function (objItem) {
                    ...
                    */
                    ...
                    //----====|| SORT INTO COLUMNS ||====----\\
                        for(var i=0;i<arrWordSets[0].phrases.length;i++){ 
                            if(filtered==false && objItem.text.toLowerCase().indexOf(arrWordSets[0].phrases[i].toLowerCase())!= -1){
                                objItem.analysis.filtered=arrWordSets[0].phrases[i];
                                //console.log(objItem.text.match(/arrWordSets[0].phrases[i]/i));
                                //console.log(arrWordSets[0].phrases[i]+' : '+objItem.text);
                                filtered = true;
                            } 
                        }
